...
この節では, Coq の基礎となっている理論である$\lambda$計算について\cite{Bar}を基に説明する. 初めは型無しの$\lambda$計算について述べ, その後に型付き$\lambda$計算の説明をし, 最後の節で Coq との関係について触れる. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$\lambda$2とは別の拡張として, $\lambda$Pがある. Pは predicate のPであり, 述語論理に対応する. $\lambda$Pの特徴は依存関数型を扱えることと, 命題の証明のシュミレートを行えることである. これからは項と型を区別せず, まとめて擬式と呼ぶことにして, 以下のように定義する. 
...
\eex
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{$\lambda$-cube と Curry-Howard 同型}
...
\begin{itemize}
  \item $\lama$
  \item $\lambda$2
  \item $\lambda \ul{\omega}$
  \item $\lambda \omega$ ($\lambda 2 + \lambda \ul{\omega}$)
  \item $\lambda$P
  \item $\lambda$P2
  \item $\lambda$P$\ul{\omega}$
  \item $\lambda$P$\omega$ ($\lambda 2 + \lambda \ul{\omega}$ + $\lambda$P)
\end{itemize}
を考えることができ, これらをまとめて$\lambda$-cube と呼んでいる(\cite{Bar} p77, 78). $\lambda$-cube のうち最も強い$\lambda$P$\omega$に帰納的型を導入した$\lambda$計算が Coq で採用されている. 
...
\end{align*}
という対応関係である. $\lambda$Pの説明の際にも触れたが, この対応関係を基に命題の証明をシュミレートすることが形式化であり, Coq に型付き$\lambda$計算が関係する理由である. 
...
と思うことができる. ここで$P$の要素に$a$, 関数型$P \to Q$の要素(つまり関数)を$f$と名前をつてみると, $f$に$a$を適用した$f a$にはどんな型がつくであろうか. $\lama$の導出規則を思い出してみると, ($\to$除去)が使えて$f a$には$Q$という型がつく. よって型として$Q$を持つような要素として具体的に$f a$という項が構成できたため, $Q$が成り立つことを証明できたことになる. 
\eex
...
ここでは定理証明支援系 Coq の使い方を, 具体例を用いて説明する. また, この後の形式化でよく使う型について, その構造の説明も行う.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
