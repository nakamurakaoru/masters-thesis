...
Theorem modus_ponens (P Q : Prop) : (P -> Q) /\ P -> Q. \end{lstlisting}
...
\end{screen}
これで証明が終了したので, {\tt Qed}を書くことで補題として登録されることになる. \\
...
\begin{lstlisting}{Coq}
Theorem expand n : 2 * (n + 1) = n + n + 2. \end{lstlisting}
このとき, ゴールエリアは以下の通りである. 
\begin{screen}
  \begin{lstlisting}{Coq}
    n : nat
    -------------------------------
    2 * (n + 1) = 2 * n + 2 \end{lstlisting}
\end{screen}
まず, 左辺を分配法則で書き換えたい. mathcomp の ssrnat に{\tt mulnDr}という, \\
{\tt forall (x y z : nat),  x * (y + z) = x * y + x * z}\\
という等式にあたる補題があるため, {\tt rewrite mulnDr}でゴールを書き換える.
\begin{screen}
  \begin{lstlisting}{Coq}
    n : nat
    -------------------------------
    2 * n + 2 * 1 = 2 * n + 2 \end{lstlisting}
\end{screen}
次に, {\tt 2 * 1}を{\tt 2}に書き換えたい. {\tt muln1}という補題が 
{\tt forall n, n * 1 = n}
にあたるので, {\tt rewrite muln1}で書き換える. \\

\begin{screen}
  \begin{lstlisting}{Coq}
    n : nat
    -------------------------------
    2 * n + 2 = 2 * n + 2 \end{lstlisting}
\end{screen}
このとき, ゴールは全く同じもの同士の等号であるため, 自明に成り立つ, つまり{\tt by []}で終了する. 
\begin{screen}
  \begin{lstlisting}{Coq}
    No more subgoals.  \end{lstlisting}
\end{screen}
...
\brmk
  正確には, {\tt mulnDr}は
  \[
    {\tt right\_distributive\ muln\ addn}
  \]
  という補題であり, {\tt muln}と{\tt addn}はそれぞれ自然数同士の乗法と加法である.  
  {\tt right\_distributive}の定義は
  \begin{lstlisting}{Co}
fun (S T : Type) (op : S -> T -> T) (add : T -> T -> T) =>
forall (x : S) (y z : T), op x (add y z) = add (op x y) (op x z) \end{lstlisting}
  であり, 分配法則を一般的に定義している. {\tt mulnDr}は{\tt right\_distributive}の{\tt op}に    
  {\tt muln}, {\tt add}に{\tt addn}を入れたものであるので,
  \begin{lstlisting}{Coq}
forall (x : nat) (y z : nat), muln x (addn y z) = addn (muln x y) (muln x z) \end{lstlisting}
  となる. 同様に, {\tt muln1}の定義も正確には
  \begin{lstlisting}{Coq}
right\_id 1 muln \end{lstlisting}
  であり, {\tt right\_id}の定義は
  \begin{lstlisting}{Coq}
fun (S T : Type) (e : T) (op : S -> T -> S) => forall x : S, op x e = x \end{lstlisting}
  であり, {\tt e}に{\tt 1}, {\tt op}に{\tt muln}を入れると
  \begin{lstlisting}{Coq}
forall x : nat, muln x 1 = 1 \end{lstlisting}
  となる. 
\ermk
このように, Coqでの証明は, ゴールを自明な形になるまで簡単な形に書き変えることを繰り返すやり方が基本である. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{$q$-類似}
...
\brmk
  ...
  なっている. 
\ermk
\cite{Kac}では証明は1行で終わっているが, 形式化する場合には何倍もかかっている. これは, 積の交換法則や指数法則などの, 通常の数学では当たり前なことが自動では計算されず, {\tt rewrite mulrC}や{\tt rewrite expfzDr}というように{\tt rewrite}での書き換えを明示的に行わなければならないからである.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ここまでは\cite{Kac}の定義 function に沿って関数に対して$q$-微分を定義し, また関数として
