...
$q$-類似の概要
実数パラメータ$q$を'極限で$1$に近づけると通常の数学に一致するような一般化
自然数ならば
$1$, $1 + q$, $1 + q + q^2$が$1$, $2$, $3$, $\ldots$に対応する. 
...
人間がチェックすることが難しい複雑な証明でも正しさが保証され, また証明付きプログラミングにも応用される. 実際にCoqが用いられた有名な例として, 四色定理やフェイト・トンプソンの定理(奇数位数定理)などがある. 今回の証明に関しては, Coqの標準ライブラリ(\cite{coq sl})に加えて, 数学の証明のために整備されたライブラリである mathcomp(\cite{coq mc}) も用いている. Coq や mathcompの使い方については, \cite{Hag}等を参照のこと. 
