...
{\tt forall m n l,  m * (n + l) = m * n + m * l}
にあたる補題があるため, {\tt rewrite mulnDr}でゴールを書き換える. 
(正確には, {\tt mulnDr}は{\tt right\_distributive muln addn}という補題である. {\tt muln}と{\tt addn}はそれぞれ自然数同士の乗法と加法であり. {\tt right\_distributive}は)
\begin{screen}
  \begin{lstlisting}{Coq}
    n : nat
    -------------------------------
