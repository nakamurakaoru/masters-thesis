...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{序文}
本論文は, $q$-類似の基礎的な結果をCoqによって形式化するものである. 具体的には\cite{Kac}の5章前半, (5.5)式の形式化までを扱う. 
...
というように形式化できる. 
\brmk
  {\tt c != 0}という条件は約分のためのものであるが, {\tt q != 0}はなぜ必要なのであろうか. 
...
Proof.
  ...
  by apply mulf_neq0 => //.
Qed. \end{lstlisting}
  の証明において, {\tt IH}を使う際に{\tt q * x != 0}という条件が必要だからである. 
  さらに, 高階{\tt Dq}が定義できないという点からも{\tt q != 0}という条件は妥当である.
  実際, $q = 0$のとき, 
  \begin{align*}
  ...
  この問題が起きるのは{\tt dq}を値の代入を用いて定義しているからであり, 
...
Definition q_bicoef n j := q_fact n / (q_fact j * q_fact (n - j)).
Theorem Gauss_binomial' a n : (forall n, q_fact n != 0) ->
  qbinom_pos_poly (-a) n =
  \sum_(0 <= i < n.+1) (q_bicoef n i * q ^+ ((n - i) * (n - i - 1))./2 * a ^+ (n - i)) *: 'X^i.
Theorem Gauss_binomial a n : (forall n, q_fact n != 0) ->
  qbinom_pos_poly (-a) n =
  \sum_(0 <= i < n.+1) (q_bicoef n i * q ^+ (i * (i - 1))./2 * a ^+ i) *: 'X^(n - i).
Qed. \end{lstlisting}
となる. 
\brmk
