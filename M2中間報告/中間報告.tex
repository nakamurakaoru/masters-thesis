...
Qed.
\end{lstlisting}
\cite{bib1}においては, この補題は Example(\cite{bib1} p2 (1.7)) として扱われ, 
\[
  D_q x^n = \frac{(qx)^n - x^n}{(q-1)x} = \frac{q^n - 1}{q - 1} x^{n - 1}
\]
というように1行で終わっているが, 形式化する場合には何倍もかかっている. これは, 積の交換法則や分配法則といった「当たり前」の部分も明示的に書く必要があるからである. 証明中に多く出現している{\tt rewrite}は等式の形をしている補題を使って証明すべき結論を書き換えるタクティックであり, {\it lem}が$A = B$という形の補題のとき, {\tt rewrite} {\it lem}で結論に現れる$A$を$B$に変える. \\
$a \in \R$, $n \in \N$に対して, $(x - a)^n$の$q$-類似は以下のように形式化できる. 
\begin{lstlisting}{Coq}
...
\[
  (x - a)^n_q = \begin{cases}
                    ...
                    \end{cases}
\]
と定義している(\cite{bib1} p8 Definition (3.4))が, 形式化する際には{\tt Fixpoint}を用いて, 再帰関数として定義している. この qpoly\_nonneg について, 
\begin{lstlisting}{Coq}
